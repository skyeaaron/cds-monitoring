...
if 'end_date' in config_dict:
    ...
    if 'start_date' in config_dict:
        ...
        frequency = 'custom'
        print('Custom frequency. Using start and end dates from config file.')
    elif 'frequency' in config_dict:
        frequency = config_dict['frequency']
        start_date, end_date = df.calc_start_and_end(end_date, frequency)
        print('Using end date and frequency from config file.')
    else:
        frequency = 'daily'
        start_date, end_date = df.calc_start_and_end(end_date, frequency)
        print('Using end date from config file with daily frequency.')
elif 'frequency' in config_dict:
    frequency = config_dict['frequency']
    start_date, end_date = df.calc_start_and_end(df.today_str(), frequency)
    print('Using frequency from config file. Default end date is today.')
else:
    frequency = 'daily'
    start_date, end_date = df.calc_start_and_end(df.today_str(), frequency)
    print('End date and frequency not specified in config. Using today and yesterday (daily).')
